...
public class MediscreenDiabetesRapportModule {
	...
	public PatientProxy getPatientProxy() {
		return Feign.builder().decoder(new JacksonDecoder()).target(PatientProxy.class, "http://patient:8081");
	}
	
	@Bean
	public PatientHistoryProxy getPatientHistoryProxy() {
		return Feign.builder().decoder(new JacksonDecoder()).target(PatientHistoryProxy.class, "http://patienthistory:8082");
	}
